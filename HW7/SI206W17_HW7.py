...
#print(get_user_tweets("ameliacacchione"))
...
table_spec += 'author TEXT, time_posted TEXT, tweet_text TEXT, retweets INTEGER)'
cur.execute(table_spec)
'''

cur.execute('DROP TABLE IF EXISTS Tweets')
cur.execute('CREATE TABLE Tweets (id INTEGER PRIMARY KEY, tweet_id INTEGER, author TEXT, time_posted TIMESTAMP, tweet_text TEXT, retweets INTEGER)')
...
for x in range(len(umsi_tweets)):
	tweet_id = umsi_tweets[x]["user"]["id"]
	author = umsi_tweets[x]["user"]["screen_name"]
	time_posted = umsi_tweets[x]["created_at"]
	tweet_text = umsi_tweets[x]["text"]
	retweets = umsi_tweets[x]["retweet_count"]

	cur.execute(statement, (None, tweet_id, author, time_posted, tweet_text, retweets))
...
q3 = "SELECT tweet_text FROM Tweets WHERE retweets > 1"
...
def get_twitter_users(tweet):
	...
	answer = re.findall(regex, tweet)

	names = set()

	for tweet in answer:
		stripped_tweet = tweet[0].strip('@')
		names.add(stripped_tweet)
